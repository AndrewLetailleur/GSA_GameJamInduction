...
public class FlyingEnemyAI : MonoBehaviour
{
   ...
   void PatrolMovement(){
       if(isMovingRight){
           transform.Translate(Vector2.up * _genericStats.speed * Time.deltaTime);
       }
       else if(!isMovingRight)
       {
           transform.Translate(Vector2.down * _genericStats.speed * Time.deltaTime);
       }
   }
   ...
   void AgressiveIntance(){
       ...
       if(Vector3.Distance(transform.position, _target.position) < _keepDistance){
           transform.position = Vector3.MoveTowards(transform.position, new Vector3(-(_target.position.x), transform.position.y, 0f), _genericStats.speed * Time.deltaTime);
